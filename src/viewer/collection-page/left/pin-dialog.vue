...
import { PinModel, DialogType } from '@/interface';

const props = defineProps({
    show: Boolean,
    type: { type: Number, default: DialogType.ADD},
    item: PinModel
})
...
const create = () => emits('complete', { name: info.name, color: info.color });
...
watch(() => props.show, () => {
    if (props.show && props.type === DialogType.EDIT) {
        info.color = props.item?.color || '#000';
        ...
        <main class="modal-box relative text-base-content" style="max-width: 24rem;" @click.stop.prevent="">
            ...
            <section class="leading-9 relative">
                ...
                <div class="h-12 mt-3 flex items-center input border-opacity-20 cursor-pointer" @click="curStep = 2">
                    ...
                    <div class="w-6 h-6 rounded-lg bg-black mr-2" :style="{background: info.color}"></div>
